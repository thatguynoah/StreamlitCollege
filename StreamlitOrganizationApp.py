...
class Group():
    def __init__(self, region, state, selectivity, status, size, colleges):
        self.region = region
        self.state = state
...
# IMPORT THE DATA
df = pd.read_csv(r"REDUCED_DATAV2.csv", index_col="INSTNM", low_memory=False)
...
with select:
    ...
    if createGroup:
        ...
        group = Group(region, state, admission, status, size, colleges)
        st.session_state[f"group{len(st.session_state)+1}"] = group
    deleteGroups = st.button("Delete All Groups")
    if deleteGroups:
        for key in st.session_state.keys():
            del st.session_state[key]
    st.text(f"Number of groups: {len(st.session_state)}")
